...
const launch = (fixture) => {
  return fork(resolve(__dirname, fixture), [], {
    execArgv: process.env.NYC_ROOT_ID
      ? process.execArgv
      : ['-r', 'ts-node/register'],
  });
};
    ...
    it('should receive data from metric', (done) => {
      ...
      child.on('message', (res) => {
        if (res.type === 'axm:monitor') {
          ...
          expect(
            res.data.hasOwnProperty('metric wi!th special chars % ///')
          ).to.equal(false);
  ...
  describe('Actions', () => {
    it('should receive data from action', (done) => {
      ...
      child.on('message', (res) => {
        if (
          res.type === 'axm:action' &&
          res.data.action_name === 'testAction'
        ) {
    ...
    it('should receive data from action with conf', (done) => {
      ...
      child.on('message', (res) => {
        if (
          res.type === 'axm:action' &&
          res.data.action_name === 'testActionWithConf'
        ) {
          child.send(res.data.action_name);
        } else if (res.type === 'axm:reply') {
  ...
  describe('InitModule', () => {
    ...
    it('should receive data from init module', (done) => {
      ...
      child.on('message', (pck) => {
        if (
          pck.type === 'axm:option:configuration' &&
          pck.data.module_name === 'fixtures'
        ) {
          const conf = pck.data;
